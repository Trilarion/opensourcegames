...
if __name__ == "__main__":
    ...
    for file in c.screenshots_path.iterdir():
        ...
        with Image.open(file) as im:
            ...
            if sz[1] != HEIGHT:
                print(f'Screenshot with unexpected height: {file} {sz[1]}')
    ...
    print(f'Entries with screenshots {len(overview)}')
    ...
    ba = b - a
    if ab:
        print(f'Names in screenshot files but not in overview: {ab}')
    if ba:
        print(f'Names in overview but not in screenshot files: {ba}')
